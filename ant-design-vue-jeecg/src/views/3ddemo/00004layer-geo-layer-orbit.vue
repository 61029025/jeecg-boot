  ...
  mounted() {
    ...
    viewer.imageryLayers.addImageryProvider(
      new WebMapTileServiceImageryProvider({
        url:
          tdtUrl+"ter_w/wmts?service=wmts&request=GetTile&version=1.0.0&LAYER=ter&tileMatrixSet=w&TileMatrix={TileMatrix}&TileRow={TileRow}&TileCol={TileCol}&style=default.jpg&tk=" + token,
    ...
    viewer.imageryLayers.addImageryProvider(
      new WebMapTileServiceImageryProvider({
        url:
          tdtUrl + "ibo_w/wmts?service=wmts&request=GetTile&version=1.0.0&LAYER=ibo&tileMatrixSet=w&TileMatrix={TileMatrix}&TileRow={TileRow}&TileCol={TileCol}&style=default.jpg&tk=" + token,
        subdomains: subdomains,
        layer: "tdtAnnoLayer",
        style: "default",
        format: "tiles",
    ...
    // // 将三维球定位到中国山西
    // viewer.camera.flyTo({
    //   destination: Cartesian3.fromDegrees(111.208091, 37.091119, 800000),
    //   orientation: {
    //     heading: Math.toRadians(0),//方向
    //     pitch: Math.toRadians(-89.74026687972041),//倾斜
    ...
    var center = Cartesian3.fromDegrees(111.208091, 37.091119, 0);
    var transform = Transforms.eastNorthUpToFixedFrame(center);
    viewer.scene.camera.lookAtTransform(transform, new HeadingPitchRange(0, -Math.PI/8, 200000));
